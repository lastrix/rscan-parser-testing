...
sealed class RModifierOp private
(
  key: ROpKey,
  ...
  override def prettyPrint(sb: StringBuilder, prefix: String): Unit = {
    ...
    sb.append(prefix).append('\t').append("Modifier: ").append(modifier.text)
    sb.append(LINE_SEPARATOR)
    appendChildren(sb, prefix)
  }

  override def specText: String = modifier.text
}

object RModifierOp {
  @NotNull
  def apply(@NotNull statement: Statement,
            @NotNull modifier: RModifier): RModifierOp =
    new RModifierOp(ROpKey(StdOpType.MODIFIER, statement), modifier)
