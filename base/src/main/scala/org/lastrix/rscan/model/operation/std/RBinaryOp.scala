...
sealed class RBinaryOp private
(
  key: ROpKey,
  val binaryType: BinaryType,
  operands: Iterable[ROp]
)
  extends ROp(key, operands) {

  override def prettyPrint(sb: StringBuilder, prefix: String): Unit = {
    sb.append(prefix).append(key).append(LINE_SEPARATOR)
    sb.append(prefix).append('\t').append("BinaryType: ").append(binaryType).append(LINE_SEPARATOR)
    appendChildren(sb, prefix)
  }

  override def specText: String = binaryType.name()
}

object RBinaryOp {
  @NotNull
  def apply(@NotNull statement: Statement,
            @NotNull `type`: BinaryType,
            @NotNull operands: Iterable[ROp]): RBinaryOp =
    new RBinaryOp(ROpKey(StdOpType.BINARY, statement), `type`, operands)
