...
object AnyFileAnalyzer extends FileAnalyzer {
  ...
  override def analyze(vfs: VirtualFileSystem, file: File, actualName: String): Unit = {
    ...
    if (vfs.fileFilter.isAllowed(root.absoluteName, name))
      root.add(new VirtualFile(name, new FileSource(actualFile, name), root))
  }
...
sealed class AnyFileAnalyzerFactory extends FileAnalyzerFactory {
  override def newInstance(): FileAnalyzer = AnyFileAnalyzer
}
